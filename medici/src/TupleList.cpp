...
int TupleList::checkTuples(TupleList list) {
	...
	for (std::list<Tuple>::iterator it = list.tList.begin(), end =
		...
		for (std::list<Tuple>::iterator it2 = (it), end = list.tList.end();
			...
			if (res.size() == nWise) //se sono identiche l'intersezione � uguale all'inizio
					{
...
//	cout<<"tupla: "<<t.getValue()<<" coperta"<<endl;
	/*cout<<"**Cercando TUPLA "<<&t<<endl;
	 ...
	 {
	 //nodo attuale
	 if (it->status>=0) //Se � gi� coperta non serve  //TODO invece se ci sono ancora tuple copribili va considerato
	 {
...
//	{
	...
	//fino a qui � ordinato per index parametri
	weightMax = 0;
	for (std::list<Tuple>::iterator it = tList.begin(), end = tList.end();
			it != end; ++it) {
		//assegno peso a tuple
		it->weight = 0;
		it->weightMax = 0; //peso del parametro pi� pesante
		for (std::vector<int>::iterator it2 = it->param.begin(), end =
				it->param.end(); it2 != end; ++it2) {
			...
			if (indexList[*it2][1] > it->weightMax) {
				it->weightMax = indexList[*it2][1]; //NB in [][0] c'� l'indice del parametro che prima del riordino � guauale a quello del vettore
			}
		}
...
void TupleList::sortCompatibility(int mode) {
	...
	if (mode == 4) {
		...
		//provo a mettere solo in cima il pi� pesante
	...
	for (std::list<Tuple>::iterator it = tList.begin(), end = tList.end();
		...
		for (std::vector<int>::iterator it2 = it->paramCASA.begin(), end =
			...
			if (indexListCASA[*it2][1] > it->weightMax) {
				it->weightMax = indexListCASA[*it2][1]; //NB in [][0] c'� l'indice del parametro che prima del riordino � guauale a quello del vettore
			}
		}
...
//	int nchecks=0;
	...
	for (std::list<Tuple>::iterator it = tList.begin(), end = tList.end();
			it != end; ++it) {
		if ((it->status==0)) //Se � gi� coperta non serve
		{
			...
			for (std::list<Tuple>::iterator it2 = it, end = tList.end();
						it2 != end; ++it2) {
				if ((it2->status==0)&&(it!=it2)){ //se � gi� coperta non serve, idem non la confronto con se stessa
					//dd_edge node2 = Operations::getMDDFromTuple(it2->code,mdd);
					//dd_edge node2 = Operations::getMDDFromTuple({0,0,-1},mdd);
					//-1 se non compatibili, >0 se compatibili, nota che potrei controllare che ogni altra combinazione � impossibile, per ottimizzare
