...
// da FCC >= 4.7 uint non � pi� supportato
#ifndef uint
...
void MDDList::updateCovered(int mddIndex, TupleList &listT, int mode) {
	...
	for (std::list<Tuple>::iterator it = listT.tList.begin(), end =
		...
		if (!it->status) //Se � gi� coperta non serve
		{
...
int MDDList::checkForParameter(vector<int> it2, Tuple &tupla, int* bounds,
	...
	bool debug = false;
	int var; //= mdd->getDomain()->getVariableBound(paramI+1,false); //cardinalit� parametro paramI
	int paramI = it2.at(0); //index originale, mentre a 1 c'� il peso associato
...
//	int maxVarStep1Value = -1; //valore del parametro che inserisce pi� tuple
...
//	vector<int> maxVarStep2ValueS;
	...
	for (int i = 0; i < var; i++) //testo possibili valori dei parametri
		...
		for (std::list<Tuple>::iterator it2 = listT.tList.begin(), end =
			...
			if (it2->code[paramI] != i)
				continue; //non � compatibile o non include il parametro
			if (toCheck) {
			...
			//Questo test impiega troppo tempo node2 � la tupla e ok, nodeTmp � il modello con constraints, provare con uno vuoto solo con parametri fissati?
			tupla.code[paramI] = i; //cambio temporaneo per check
			int isIncluded = Operations::isIncludedTuple(tupla.code, it2->code,
					it2->param);
			tupla.code[paramI] = -1;
			if (isIncluded == 1) { //Gi� inclusa
				...
				if (countVarStep1 <= 0) //non eseguo se c'� gi� step1 fissato, ottimizza per� lo usavo in caso di parit� di countvar1... qualche dato per� mi resta perch� qualche volta lo eseguo lo stesso
						{
					...
					} //primo parametro � sempre compatibile
					else
		...
		if (countVarStep1 > 0) {
			if (countVarStep1 > maxVarStep1) {
				...
				/*
				 ...
				 //maxVarStep1=countVarStep1; //non serve � gi� uguale per ipotesi
				 if (countVarStep2>maxVarStep2)
				 maxVarStep1ValueS.clear();
				 maxVarStep1ValueS.push_back(i);
				 //maxVarStep1Value = i;
	...
	//uscito dal ciclo avr� un valore da fissare per il parametro
	...
	if ((maxVarStep2 + maxVarStep1) == 0)
		return 0; //-1 se fissare quel parametro non copre e non coprir� mai niente
	if (maxVarStep1 > 0)
		return maxVarStep1;
	else
...
//					nodoVuoto.show(stdout,3);
			...
			//	cout<<"------------"<<endl;
			//cout<<"Cardinalit� codice "<<it2->value<<" uguale a: "<<tP.getCardinality()<<endl;
			if (it2.isNegative()) {

				//	cout<<"segno negativo "<<endl;
				//	cout<<"Cardinalit� nodo vuoto "<<nodoVuoto.getCardinality()<<endl;
				//			nodoVuoto.show(stdout,3);
				tP = nodoVuoto - tP; //fare il complementare
				//	cout<<"Cardinalit� codice "<<it2->value<<" modificata da not uguale a: "<<tP.getCardinality()<<endl;
			}
		...
		//c.show(stdout,3);
		logcout(LOG_INFO) << "Cardinalit� del constraint " << c.getCardinality() << endl;

		listConstraints.push_back( { c, params }); //params � la lista dei parametri che include il constraint
...
//	createConstraintsMddList(tList.constraintsIndexList); //TODO tolto non cambiava i tempi, la frammentazione � il massimo
//	nodoBase.show(stdout,3);
...
void MDDList::updateConstraintsMEDICI(
	...
	for (std::vector<std::list<Operations::TestModelConstraint> >::iterator it =
		...
		//ho il mio constraint
		//c.show(stdout,3);
		//cout<<"Cardinalit� del constraint "<<c.getCardinality()<<endl;
...
//	lCNew.push_back(*listConstraints.begin()); //inizializzo mettendo il primo elemento
	for (std::vector<MDDList::Constraint>::iterator it =
		...
		//{
		//controllo se ho gi� inserito un mdd compatibile -> iterator su lCNew NB il primo lo mette pure se c'� gi� ma fa niente
		...
		for (std::vector<MDDList::Constraint>::iterator it3 = lCNew.begin(),
			...
			if (res.size() > 0) {
				...
				//� compatibile aggiungo qui
				//NB potrebbe essere compatibile con pi� di uno e in quel caso devo mischiarli di nuovo, ripassando il ciclo
				...
				//RIORDINO ed elimino doppi, merge elimina gi� doppi e probabilmente riordina anche
				break;//non mi serve andare avanti ho trovato compatibile
			}

		}
		if (!comp) //non � compatibile con nessuno, aggiungo nuovo
		{

			//cout<<"NON � compatibile con nessuno creo nuovo"<<endl;
			lCNew.push_back(*it);
		}
		//}
	}
...
int MDDList::checkCoverable(forest* mdd, TupleList &tupleList) {
	...
	for (std::list<Tuple>::iterator it = tupleList.tList.begin(), end =
		...
		//Aggiungo check preliminare che pu� evitare il resto del controllo
...
int MDDList::listToFile(char* filename, int *bounds) {
	...
	for (uint z = 0; z < list.size() - 1; z++) //ultimo � sempre uno vuoto
			{
		vector<cvalue> code = Operations::getCodeFromMdd(list[z].node);
		Operations::toCASACodeConversion(code, bounds);
		for (uint k = 0; k < code.size(); k++) //ultimo � sempre uno vuoto
				{
			fout << code[k] << " ";
		}
		fout << endl;
...
int MDDList::addOther2b(TupleList &listT, int* bounds, int tries) //bounds non doveva servire ma ci deve essere un bug in meddly;
	...
	for (std::list<Tuple>::iterator it = listT.tList.begin(), end =
		...
		if (it->status == 1)
			continue; //� gi� coperta
		//non � coperta quindi da questa genero sicuramente un caso di test
		if (tI == 0) {
			//Aggiungo mdd
			//prima ne aggiungo uno vuoto in coda

			//ora aggiungo a quello che � ora il penultimo elemento la tupla
			aggiungiAt(mddIndex, it->code); //TODO da rimettere
			listT.setCovered(*it); //� in mdd quindi setto coperta
			list[mddIndex].nCoveredMdd++;
			list[mddIndex].tWeight += it->weight;
			double wmax = 0;
			...
			//ho gi� una tupla inserita, test compatibilit�
	...
	for (int z = 0; z < tries; z++) {
		...
		while (((indexListTC.size() > 0) && (coverable == true))
			...
			indexListTC.clear();
			/**
			 * RIORDINARE indexListTMP mettendo prima i parametri che interagiscono con i gi� fissati
			 ultimo dei fissati, il pi� pesante, solo una volta?
			 */
			...
			for (std::vector<vector<int> >::iterator it2 = indexListTMP.begin(),
				...
				if (tupla.code[paramI] > -1)
					continue; //parametro gi� fissato
				//trovato parametro non fissato
				if (it2->at(1) == 0) {
					//non coinvolge nessuna tupla e quindi essendo ordinati � cos�
				...
				}		//salto senza riprenderlo, fissarlo � controproducente
				if ((nCoverable > 0) || ((paramIt < 1) && (paramZero > 0))) {
					...
					it2->at(1) = (0 - nCoverable); //assegno preso secondario e le riodino cos�
...
#endif
					tupla.code[paramI] = -1;
					coverable = true; //quelli che proprio non vanno bene sono gi� esclusi
				}
				paramIt++;
		...
		//saturo mdd a un caso di test valido anche nel caso abbia cardinalit� maggiore
		vector<cvalue> code = Operations::getCodeFromMdd(list[mddIndex].node);
